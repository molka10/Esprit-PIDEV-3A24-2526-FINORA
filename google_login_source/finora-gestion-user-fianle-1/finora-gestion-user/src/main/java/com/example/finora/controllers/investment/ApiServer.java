...
public class ApiServer {
    ...
    public static void start() {
        try {
            ...
            System.out.println(
                    "⚠️ Recommendations API server is already running on port 7777 (Background port in use). Continuing...");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static String getTopInternal(int limit) {
        ...
        List<Map<String, Object>> ranked = investments.stream()
                .map(inv -> {
                    ...
                    double normalizedValue = (value - minValue) / (maxValue - minValue + 1);

                    double riskScore = mapRiskToScore(inv.getRiskLevel());

                    double score = (0.6 * normalizedValue) +
                            (0.4 * riskScore);
    ...
    private static String getTopExternal(int limit) {
        ...
        List<Map<String, Object>> ranked = investments.stream()
                .map(inv -> {
                    ...
                    double trend = getExternalTrendScore(inv.getCategory());
