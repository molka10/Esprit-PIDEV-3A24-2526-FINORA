...
import com.example.finora.services.IServices;
 ...
 * 
 * BONUS IMPORTANT :
 * - getByIdWithActions(id) charge une bourse + ses actions (relation
 * bidirectionnelle)
 */
public class ServiceBourse implements IServices<Bourse> {
    ...
    public void add(Bourse bourse) {
        ...
        try (PreparedStatement pst = getConnection().prepareStatement(req)) {
    ...
    public Bourse getByIdWithActions(int idBourse) {
        ...
        try (PreparedStatement pst = getConnection().prepareStatement(reqBourse)) {
            ...
            try (PreparedStatement pst2 = getConnection().prepareStatement(reqActions)) {
