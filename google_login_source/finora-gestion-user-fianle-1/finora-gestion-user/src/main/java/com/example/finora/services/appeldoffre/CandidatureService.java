...
public class CandidatureService {
    ...
    public void add(Candidature c) throws SQLException {
        ...
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ...
            if (c.getMontantPropose() == 0)
                ps.setNull(4, Types.DOUBLE);
            else
                ps.setDouble(4, c.getMontantPropose());
    ...
    public List<Candidature> getByEmail(String email) throws SQLException {
        List<Candidature> list = new ArrayList<>();

        String sql = "SELECT * FROM candidature WHERE email_candidat=? ORDER BY candidature_id DESC";
