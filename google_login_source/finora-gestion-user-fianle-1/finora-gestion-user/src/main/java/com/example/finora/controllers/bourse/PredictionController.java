...
import com.example.finora.services.bourse.ServicePrediction;
import com.example.finora.services.bourse.ServicePrediction.ResultatPrediction;
...
public class PredictionController implements Initializable {
    ...
    @FXML
    private ComboBox<Action> cbAction;
    @FXML
    ...
    // Info action
    @FXML
    private Label lblActionNom;
    @FXML
    private Label lblSecteur;
    @FXML
    ...
    @FXML
    private Label lblPred7j;
    @FXML
    private Label lblPred30j;
    @FXML
    private ProgressBar progressConfiance;
    @FXML
    private Label lblConfiance;

    // Scénarios
    @FXML
    private Label lblScenarioOpt;
    @FXML
    private Label lblScenarioReal;
    @FXML
    private Label lblScenarioPess;

    // Analyse
    @FXML
    private TextArea txtAnalyse;
    ...
    private String getCouleurTendance(String tendance) {
        if (tendance == null)
            return "#6b7280";
        if (tendance.toLowerCase().contains("haussi"))
            return "#10b981";
        if (tendance.toLowerCase().contains("baissi"))
            return "#ef4444";
        return "#f59e0b";
    }
    ...
    private void afficherGraphique(Action action, ResultatPrediction resultat) {
        ...
        Platform.runLater(() -> {
            // Historique en gris
            lineChart.lookupAll(".series0")
                    .forEach(node -> node.setStyle("-fx-stroke: #6b7280; -fx-stroke-width: 2px;"));
            // Prédiction en bleu pointillé
            lineChart.lookupAll(".series1").forEach(
                    node -> node.setStyle("-fx-stroke: #6366f1; -fx-stroke-width: 3px; -fx-stroke-dash-array: 5 5;"));
        });
    }
    ...
    private void retourDashboard(ActionEvent event) {
        ...
        Navigator.goTo(stage, "/bourse/dashboard-investisseur-view.fxml", "Dashboard Investisseur");
    }
