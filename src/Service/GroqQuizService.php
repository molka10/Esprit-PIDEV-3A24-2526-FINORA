...
class GroqQuizService
{
     ...
     * }>
     */
    public function generateQuiz(string $lessonTitle, string $lessonContent, string $lang = 'fr'): array
    {
        ...
        $instructions = [
            'fr' => "Tu es un professeur. Génère EXACTEMENT 5 questions QCM (4 choix) en français.",
            'en' => "You are a teacher. Generate EXACTLY 5 multiple-choice questions (4 options) in English.",
            'ar' => "أنت معلم. قم بإنشاء 5 أسئلة من نوع الاختيار من متعدد بالضبط (4 خيارات) باللغة العربية."
        ];

        $langPrompt = $instructions[$lang] ?? $instructions['fr'];

        $prompt = <<<PROMPT
{$langPrompt}
Basées sur la leçon ci-dessous.
...
PROMPT;
    ...
    public function explainQuestion(string $lessonContent, string $question, string $correctAnswer, string $userAnswer, string $lang = 'fr'): string
    {
        $content = $this->safe($lessonContent);
        if (mb_strlen($content) > self::MAX_CONTENT_CHARS) {
            $content = mb_substr($content, 0, self::MAX_CONTENT_CHARS);
        ...
        $instructions = [
            'fr' => "Tu es un professeur bienveillant. Explique en français.",
            'en' => "You are a kind teacher. Explain in English.",
            'ar' => "أنت معلم لطيف. اشرح باللغة العربية."
        ];
