...
class PortfolioAnalyticsService
{
    ...
    public function getDashboardStats(array $allInvestments, array $allManagements): array
    {
        ...
        $riskScores = ['LOW' => 30, 'MEDIUM' => 60, 'HIGH' => 90];
        $totalRisk = 0;
        $count = 0;
        foreach ($activeManagements as $m) {
            if ($m->getInvestment()) {
                $totalRisk += $riskScores[$m->getInvestment()->getRiskLevel()] ?? 60;
                $count++;
            }
        }
        $avgRisk = $count > 0 ? round($totalRisk / $count) : 0;

        return [
            'totalProjects'       => count($allInvestments),
