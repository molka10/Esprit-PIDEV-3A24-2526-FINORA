...
import com.example.finora.services.appeldoffre.CurrencyApiService;
import com.example.finora.services.appeldoffre.EmailService;
import com.example.finora.services.appeldoffre.HolidayApiService;
import com.example.finora.services.appeldoffre.OpenRouterDescriptionService;
...
public class AppelOffreFormController {
    ...
    @FXML
    private Label convertedMinLabel;
    @FXML
    private Label convertedMaxLabel;
    @FXML
    ...
    public void setAppelOffre(AppelOffre a) {
        ...
        this.current = a; // Set the current field for editing
    ...
    private List<String> validateForm() {
        ...
        if (categorieCombo.getValue() == null)
            errors.add("Choisissez une catégorie.");
        if (type == null)
            errors.add("Choisissez un type.");
        if (devise == null)
            errors.add("Choisissez une devise.");
        if (statut == null)
        ...
        if (min != null && min < 0)
            errors.add("Budget min doit être ≥ 0.");
        if (max != null && max < 0)
            errors.add("Budget max doit être ≥ 0.");
        if (min != null && max != null && min > max)
            errors.add("Budget min doit être ≤ Budget max.");
    ...
    private void refreshConversion() {
        ...
        new Thread(() -> {
            try {
                ...
                if (opt.isEmpty()) {
                    Platform.runLater(() -> rateLabel.setText("Taux indisponible"));
                    return;
                }
                ...
                Platform.runLater(() -> {
                    ...
                    if (min > 0)
                        convertedMinLabel.setText("Budget min ≈ " + String.format("%.2f", min * rate) + " " + to);
                    else
                        convertedMinLabel.setText("Budget min ≈ -");

                    if (max > 0)
                        convertedMaxLabel.setText("Budget max ≈ " + String.format("%.2f", max * rate) + " " + to);
                    else
                        convertedMaxLabel.setText("Budget max ≈ -");
                });
            } catch (Exception e) {
                Platform.runLater(() -> rateLabel.setText("Erreur taux: " + e.getMessage()));
            }
        }).start();
    }
