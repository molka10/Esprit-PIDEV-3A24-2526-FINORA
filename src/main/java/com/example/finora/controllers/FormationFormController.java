...
public class FormationFormController {
    ...
    @FXML
    private TextField txtCategorieTag;
    @FXML
    ...
    public FormationFormController() {
    ...
    public void setData(Formation f) {
        ...
        if (f != null) {
            if (lblTitle != null)
                lblTitle.setText("Modifier Formation");
            ...
            if (cbNiveau != null)
                cbNiveau.getSelectionModel().select(nz(f.getNiveau()));
            parseTagsFromString(f.getCategorie()).forEach(this::addTagInternal);
        } else {
            if (lblTitle != null)
                lblTitle.setText("Ajouter Formation");
        }
        renderTags();
    }
    ...
    private void onPickCategorie() {
        ...
        if (picked == null || picked.isBlank())
            return;
        if (txtCategorieTag != null)
            txtCategorieTag.setText(picked);
        onAddTag();
    ...
    private void onSave() {
        ...
        try {
            ...
            if (titre.isEmpty()) {
                markError("Titre obligatoire");
                return;
            }
            if (niveau.isEmpty()) {
                markError("Niveau obligatoire");
                return;
            }
            if (tags.isEmpty()) {
                markError("Ajoute au moins 1 catégorie");
                return;
            }
