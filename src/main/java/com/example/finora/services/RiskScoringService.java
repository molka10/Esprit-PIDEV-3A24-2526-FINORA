...
public class RiskScoringService {
    ...
    public RiskScoringService() throws SQLException {
        this.cn = DBConnection.getInstance().getConnection();
    }

    public RiskResult compute(User user) throws SQLException {
        if (user == null) return new RiskResult(0, RiskResult.Level.LOW, List.of());
        ...
        if (score >= 60) level = RiskResult.Level.HIGH;
        else if (score >= 30) level = RiskResult.Level.MEDIUM;
        else level = RiskResult.Level.LOW;
