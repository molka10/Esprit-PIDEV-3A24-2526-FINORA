...
public class ServicePrediction {
    ...
    private static final String SYSTEM_PROMPT = "Tu es un analyste financier expert.\n" +
    ...
    private ResultatPrediction genererPredictionSimulee(Action action) {
        ...
        r.tendance = trendBase > 0.03 ? "Haussière" : trendBase < -0.03 ? "Baissière" : "Neutre";
    ...
    private ResultatPrediction parserReponse(String rep, double prixActuel) {
        ...
        if (r.tendance == null)
            r.tendance = "Neutre";
        if (r.prediction7j == 0)
            r.prediction7j = prixActuel * 1.01;
        if (r.prediction30j == 0)
            r.prediction30j = prixActuel * 1.03;
        if (r.confiance == 0)
            r.confiance = 50;
        if (r.analyse == null || r.analyse.isEmpty())
            r.analyse = rep;
        if (r.scenarioOptimiste == 0)
            r.scenarioOptimiste = prixActuel * 1.10;
        if (r.scenarioRealiste == 0)
            r.scenarioRealiste = prixActuel * 1.03;
        if (r.scenarioPessimiste == 0)
            r.scenarioPessimiste = prixActuel * 0.95;
