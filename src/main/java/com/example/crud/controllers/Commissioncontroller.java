...
import com.example.crud.entities.Commission;
import com.example.crud.services.Servicecommission;
...
public class Commissioncontroller implements Initializable {
    ...
    @FXML private Label lblTauxActuel;
    ...
    private void chargerDonnees() {
        Task<List<Commission>> task = new Task<>() {
            @Override protected List<Commission> call() {
    ...
    private HBox creerLigne(Commission c) {
        ...
        tauxLabel.setStyle("-fx-font-size: 16px; -fx-font-weight: bold; -fx-text-fill: #00d4aa; -fx-pref-width: 100; -fx-alignment: CENTER_RIGHT;");
    ...
    private void ouvrirDialog(Commission commissionAModifier) {
        ...
        grid.add(new Label("Taux (%) *"), 0, row); grid.add(tfTaux, 1, row++);
        grid.add(new Label("Statut"), 0, row); grid.add(chkActive, 1, row);
        ...
        dialog.showAndWait().ifPresent(response -> {
            ...
            if (type == null) { showError("Le type est obligatoire !"); return; }
            if (tauxStr.isEmpty()) { showError("Le taux est obligatoire !"); return; }
